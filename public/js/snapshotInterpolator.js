...
export function createSnapshotInterpolator(networkConfig, options = {}) {
    ...
    function createNetworkDiagnosticsSummary() {
        ...
        return {
            ...
            maxCaptureApplySubtractFallbacks: maxFiniteValue(snapshotEvents.map(event => getEventCaptureApply(event) && getEventCaptureApply(event).operationSubtractFallbackCount)),
            maxCaptureApplySubtractResidualOverlapArea: maxFiniteValue(snapshotEvents.map(event => getEventCaptureApply(event) && getEventCaptureApply(event).operationSubtractMaxResidualOverlapArea)),
            maxCaptureApplySubtractValidationRejected: maxFiniteValue(snapshotEvents.map(event => getEventCaptureApply(event) && getEventCaptureApply(event).operationSubtractValidationRejectedCount)),
            maxCaptureApplySubtractValidations: maxFiniteValue(snapshotEvents.map(event => getEventCaptureApply(event) && getEventCaptureApply(event).operationSubtractValidationCount)),
            ...
            maxBotSelfTrailCacheHits: maxFiniteValue(snapshotEvents.map(event => event.server && event.server.gameLoop && event.server.gameLoop.bot && event.server.gameLoop.bot.selfTrailSafety && event.server.gameLoop.bot.selfTrailSafety.safetyCacheHitCount)),
            maxBotSelfTrailCacheMisses: maxFiniteValue(snapshotEvents.map(event => event.server && event.server.gameLoop && event.server.gameLoop.bot && event.server.gameLoop.bot.selfTrailSafety && event.server.gameLoop.bot.selfTrailSafety.safetyCacheMissCount)),
    ...
    function normalizeCaptureApplyDiagnostics(value) {
        ...
        return {
            ...
            operationSubtractFallbackCount: finiteOrNull(value.operationSubtractFallbackCount),
            operationSubtractMaxResidualOverlapArea: finiteOrNull(value.operationSubtractMaxResidualOverlapArea),
            operationSubtractValidationCount: finiteOrNull(value.operationSubtractValidationCount),
            operationSubtractValidationRejectedCount: finiteOrNull(value.operationSubtractValidationRejectedCount),
    ...
    function normalizeSelfTrailSafetyDiagnostics(value) {
        ...
        return {
            ...
            safetyCacheHitCount: finiteOrNull(value.safetyCacheHitCount),
            safetyCacheMissCount: finiteOrNull(value.safetyCacheMissCount),
